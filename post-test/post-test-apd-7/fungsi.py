...
def register(pergi_register): # Fungsi untuk membuat sebuah akun
    if pergi_register == "y":
        while True:
            try:
                print("=" * 50)
                ...
                if len(username_baru) != 0 and len(password_baru) != 0:
                    print("=" * 60)
                ...
                elif len(username_baru) == 0 and len(password_baru) == 0:
                    raise ValueError("Username dan Password Tidak boleh kosong")
            
            except ValueError as e: 
                print(e)
                clear()
    elif pergi_register == "n": 
...
def tambah_game(): # Fungsi untuk menambahkan game baru ke direktori
    while True:
        try:
            ...
            if len(game_baru) != 0 and len(tahun_rilis) != 0 and len(developer_game) != 0 and len(status_game) != 0: # Jika semua data terisi, program lanjut ke bagian ini
                ...
                raise ValueError("Tidak Boleh ada yang kosong!")
                ...
                if pilih_hapus == gid:
                    while True: 
                        ...
                        if konfirmasi_hapus == "y": # Bagian ini akan menghapus game yang sudah dipilih secara permanen
                            ...
                            clear()
                            break
                        elif konfirmasi_hapus == "n": # Akan berjalan jika pengguna tidak jadi menghapus game
